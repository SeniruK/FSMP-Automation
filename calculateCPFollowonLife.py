...
def calculateCPFollowonLife(df_iat_modified_total_life, df_special_inspections, df_damagefile, df_iat_trkpt, df_cp_ref, ctrlpt_list, acsn_list):
    import pandas as pd
    import numpy as np
    ...
    for acsn in acsn_list:
        ...
        for ctrlpt in ctrlpt_list:
            ...
            if trkpt not in trkpt_list_unique:
                ...
                cp_followon_dt = df_cp_ref.loc[(df_cp_ref["CTRLPT"]==ctrlpt)].iloc[0]["Follow-on DT Life"]
                if cp_followon_dt == "SPECIAL":
                    cp_followon_dt = df_special_inspections.loc[df_special_inspections["ACSN"]==acsn].iloc[0][ctrlpt]
